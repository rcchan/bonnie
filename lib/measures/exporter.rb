  ...
  class Exporter
    def self.export(file, measures)
      ...
      Zip::ZipOutputStream.open(file.path) do |zip|      
        ...
        exporter = Measures::Exporter.new
        library_functions = exporter.library_functions
        ...
        bundle_json = exporter.bundle_json(library_functions.keys).to_json
        zip.put_next_entry(File.join(measure_path, "bundle.json"))
        zip << bundle_json

        measures.each do |measure|
          measure_json = exporter.measure_json(measure.measure_id).to_json
